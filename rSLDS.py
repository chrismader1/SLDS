...
def inference_rSLDS(px, mdl, y_test, q_train=None, z_true=None, y_train=None, dt=1/252, display=False):
    ...
    max_cpll = max_cpll_upper_bound(y_test, reg_scale=1e-8)
...
def cusum_overlay(prices, y, xhat, mdl, h_z, verbose=False):
    ...
    # init regime: choose by gates at t=0 (fallback to 0 if NaN)
    xp0 = xhat[0]
    logits0 = Rw @ xp0 + r
...
def compute_smoothed_cpll(model, x_smooth, y, gamma_smooth,
    ...
    Computations:
    CPLL = sum_{t=1}^{T-1} log N(y_t ; μ̄_t, Σ_t)
    where:
      μ_k(t) = C_k x_{t-1} + d_k
      μ̄_t   = Σ_k γ_{t-1,k} μ_k(t)
    ...
    for t in range(1, T):
        ...
        if sign <= 0:
            raise ValueError(f"Non-PD Sigma at t={t}")
...
def evaluate_rSLDS_actual(y, px, zhat, xhat, elbo, mdl, cpll, max_cpll, dt, display=True):
    ...
    L = min(len(cpll_all), len(max_cpll_all))          # NEW
    if L == 0:                                         # NEW
        cpll_best, max_cpll_paired = np.nan, np.nan    # NEW
    else:
        cpll_all, max_cpll_all = cpll_all[:L], max_cpll_all[:L]   # NEW
