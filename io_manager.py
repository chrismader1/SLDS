...
logger.propagate = False  # keep it local

def _enable_logging(level="INFO"):
...
def _disable_logging():
    ...
    logger.setLevel(logging.CRITICAL + 1)  # effectively off
...
class IOManager:
    ...
    def _peek_file(self, path, n=3, label="(peek)"):
        try:
            logger.debug(f"[IO][{label}] path={path} size={os.path.getsize(path)} bytes")
            with open(path, "r", encoding="utf-8", errors="replace") as f:
                for i in range(n):
                    line = f.readline()
                    if not line: break
                    logger.debug(f"[IO][{label}] L{i+1}: {line.rstrip()[:240]}")
        except Exception as e:
            logger.debug(f"[IO][{label}] <peek failed> {e}")

    # ---------- master init ----------
    def _ensure_master_results_csv(self):
        need_init = (not os.path.exists(self.results_csv)) or (os.path.getsize(self.results_csv) == 0)
        if need_init:
            logger.debug(f"[IO] init master results at {self.results_csv}")
    ...
    def append_temp_results(self, security, df):
        if df is None or df.empty:
            logger.debug(f"[IO][results] skip empty append for {security}")
            return
        ...
        logger.debug(f"[IO][results] append -> {tmp} rows={len(df)} header={write_header}")
    ...
    def append_temp_segments(self, security, df):
        if df is None or df.empty:
            logger.debug(f"[IO][segments] skip empty append for {security}")
            return
        df = df.reindex(columns=self.segments_header_cols)
        tmp = self._tmp_segments(security)
        write_header = (not os.path.exists(tmp)) or (os.path.getsize(tmp) == 0)
        logger.debug(f"[IO][segments] append -> {tmp} rows={len(df)} header={write_header}")
    ...
    def _append_csv_file_to_master(self, tmp_csv, master_csv):
        if not (os.path.exists(tmp_csv) and os.path.getsize(tmp_csv) > 0):
            logger.debug(f"[IO][merge] nothing to append from {tmp_csv}")
            return

        # Ensure master exists with correct header
        self._ensure_master_results_csv()

        logger.debug(f"[IO][merge] start merge tmp -> master\n  tmp={tmp_csv}\n  master={master_csv}")
        ...
        with lock:
            ...
            logger.debug(f"[IO][merge] read tmp rows={new_df.shape[0]} cols={new_df.shape[1]}")

            if new_df.shape[0] == 0:
                logger.debug("[IO][merge] tmp has 0 rows; skip")
                return
            ...
            if os.path.exists(master_csv) and os.path.getsize(master_csv) > 0:
                ...
                logger.debug(f"[IO][merge] read master rows={old_df.shape[0]} cols={old_df.shape[1]}")
            ...
            logger.debug(f"[IO][merge] wrote master rows={out.shape[0]} -> {master_csv}")
    ...
    def _append_to_parquet(self, tmp_csv, parquet_path):
        ...
        with lock:
            ...
            logger.debug(f"[IO][parquet] wrote parquet rows={df.shape[0]} -> {parquet_path}")

    def flush_one_security(self, security):
        tmp_res = self._tmp_results(security)
        tmp_seg = self._tmp_segments(security)
        logger.debug(f"[IO][flush] security={security}")
        logger.debug(f"[IO][flush] tmp_res={tmp_res} exists={os.path.exists(tmp_res)}")
        logger.debug(f"[IO][flush] tmp_seg={tmp_seg} exists={os.path.exists(tmp_seg)}")
        ...
        if os.path.exists(tmp_seg) and os.path.getsize(tmp_seg) > 0:
            ...
            logger.debug(f"[IO][flush] segments tmp rows={df.shape[0]} cols={df.shape[1]}")
            if df.shape[0] > 0:
                df.to_csv(tmp_seg, index=False, **self._WRITE_KW_SEG)
                self._append_to_parquet(tmp_seg, self.segments_parquet)
            os.remove(tmp_seg)
            logger.debug(f"[IO][flush] removed {tmp_seg}")
